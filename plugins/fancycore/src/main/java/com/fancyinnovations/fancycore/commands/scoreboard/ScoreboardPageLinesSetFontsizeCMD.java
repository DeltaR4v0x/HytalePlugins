...
public class ScoreboardPageLinesSetFontsizeCMD extends AbstractPlayerCommand {
    ...
    private final RequiredArg<Integer> fontsizeArg = withRequiredArg("fontsize", "fontsize of the line", ArgTypes.INTEGER);

    public ScoreboardPageLinesSetFontsizeCMD() {
        super("setfontsize", "Set the fontsize of a scoreboard line");
        requirePermission("fancycore.commands.scoreboard.page.lines.fontsize");
    }
    ...
    protected void execute(@NotNull CommandContext ctx, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        ...
        int fontsize = fontsizeArg.get(ctx);
        if (fontsize < 1) {
            fp.sendMessage("Invalid fontsize. It must be at least 1.");
            return;
        }
        ...
        fp.sendMessage("Set the fontsize of line " + lineNumber + " on scoreboard page '" + page.getName() + "' to " + fontsize + ".");
    }
}
