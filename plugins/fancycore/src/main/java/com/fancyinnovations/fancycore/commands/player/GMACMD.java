    ...
    protected void execute(@NotNull CommandContext ctx, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        ...
        targetWorld.execute(() -> {
            ...
            if (playerComponent.getGameMode() == GameMode.Adventure) {
                String message = "Player " + target.getData().getUsername() + " is already in adventure mode.";
                if (ctx.isPlayer()) {
            ...
            Player.setGameMode(targetRef, GameMode.Adventure, (ComponentAccessor<EntityStore>) targetStore);

            String message;
            if (target.getData().getUUID().equals(fp.getData().getUUID())) {
                message = "Your gamemode has been changed to adventure.";
            } else {
                message = "Gamemode of " + target.getData().getUsername() + " has been changed to adventure.";
            }

            if (ctx.isPlayer()) {
                fp.sendMessage(message);
                if (!target.getData().getUUID().equals(fp.getData().getUUID())) {
                    target.sendMessage("Your gamemode has been changed to adventure by " + fp.getData().getUsername() + ".");
                }
            } else {
                ctx.sendMessage(Message.raw(message));
                target.sendMessage("Your gamemode has been changed to adventure by console.");
            }
        });
    }
}
