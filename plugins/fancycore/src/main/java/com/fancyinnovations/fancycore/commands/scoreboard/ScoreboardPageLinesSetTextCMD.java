...
public class ScoreboardPageLinesSetTextCMD extends AbstractPlayerCommand {
    ...
    private final RequiredArg<String> textArg = withRequiredArg("text", "the new text", ArgTypes.STRING);

    public ScoreboardPageLinesSetTextCMD() {
        super("settext", "Set the text of a scoreboard line");
        requirePermission("fancycore.commands.scoreboard.page.lines.settext");
    ...
    protected void execute(@NotNull CommandContext ctx, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        ...
        fp.sendMessage("Set text of line " + lineNumber + " on page '" + page.getName() + "' to: " + text);
    }
}
