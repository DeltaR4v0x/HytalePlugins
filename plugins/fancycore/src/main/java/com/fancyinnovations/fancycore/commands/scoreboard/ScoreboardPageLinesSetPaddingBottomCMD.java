...
public class ScoreboardPageLinesSetPaddingBottomCMD extends AbstractPlayerCommand {
    ...
    public ScoreboardPageLinesSetPaddingBottomCMD() {
        super("setpaddingbottom", "Set the bottom padding of a scoreboard line");
        requirePermission("fancycore.commands.scoreboard.page.lines.setpaddingbottom");
    }
    ...
    protected void execute(@NotNull CommandContext ctx, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        ...
        line.setPaddingBottom(padding);
        FancyCorePlugin.get().getScoreboardStorage().storePage(page);

        fp.sendMessage("Set bottom padding of line " + lineNumber + " on page '" + page.getName() + "' to " + padding + ".");
    }
}
