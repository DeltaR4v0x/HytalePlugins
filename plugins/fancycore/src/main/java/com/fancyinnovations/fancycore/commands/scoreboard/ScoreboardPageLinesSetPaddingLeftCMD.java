...
public class ScoreboardPageLinesSetPaddingLeftCMD extends AbstractPlayerCommand {
    ...
    public ScoreboardPageLinesSetPaddingLeftCMD() {
        super("setpaddingleft", "Set the left padding of a scoreboard line");
        requirePermission("fancycore.commands.scoreboard.page.lines.setpaddingleft");
    }
    ...
    protected void execute(@NotNull CommandContext ctx, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        ...
        line.setPaddingLeft(padding);
        FancyCorePlugin.get().getScoreboardStorage().storePage(page);

        fp.sendMessage("Set left padding of line " + lineNumber + " on page '" + page.getName() + "' to " + padding + ".");
    }
}
