...
public class ScoreboardPageLinesSetPaddingRightCMD extends AbstractPlayerCommand {
    ...
    public ScoreboardPageLinesSetPaddingRightCMD() {
        super("setpaddingright", "Set the right padding of a scoreboard line");
        requirePermission("fancycore.commands.scoreboard.page.lines.setpaddingright");
    }
    ...
    protected void execute(@NotNull CommandContext ctx, @NotNull Store<EntityStore> store, @NotNull Ref<EntityStore> ref, @NotNull PlayerRef playerRef, @NotNull World world) {
        ...
        line.setPaddingRight(padding);
        FancyCorePlugin.get().getScoreboardStorage().storePage(page);

        fp.sendMessage("Set right padding of line " + lineNumber + " on page '" + page.getName() + "' to " + padding + ".");
    }
}
